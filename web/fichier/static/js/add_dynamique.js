...
function show_add_dynamique(node, select2) {
    ...
    clearErreur();
    if (node.prev().attr("id") == "select2-id_themes_m2m-results") {
        titre = "un thème";
...
function sortUsingNestedText(parent, childSelector) {
    var items = parent.children(childSelector).sort(function(a, b) {
        var vA = $(a).text().latinise().toLowerCase();
        var vB = $(b).text().latinise().toLowerCase();
...
function sort_default_list_m2m(element_class) {
    sortUsingNestedText($("#" + element_class + "_m2m"), "option");
}
